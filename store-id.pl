...
# Youtube Video
# ============================================================= #
} elsif (m/^https?\:\/\/(www|c|s)\.youtube\.com\/(ptracking|stream_204|player_204|gen_204).*(video_id|docid|v)\=([^\&\s]*).*/){
        $vid = $4 ;
        @cpn = m/[&?]cpn\=([^\&\s]*)/;
                $fn = "/tmp/@cpn";
                unless (-e $fn) {
...
} elsif (m/^https?:\/\/.*(fbstatic|profile|photo|creative)*.akamaihd\.net\/((h|)(profile|photos|ads)-ak-)(snc|ash|prn|frc[0-9])[0-9]?(.*)/) {
...
} elsif (m/^https?:\/\/fbstatic-a.akamaihd\.net\/rsrc.php\/(.*)\.(png|jpg|gif|swf|js|css|ico)/) {
        print "OK store-id=http://fbstatic.squid.internal/" . $2  . "\n";
